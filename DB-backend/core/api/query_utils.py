"""utils for parsing query string
...
from core.api.utils import ErrorCode, failed_api_response, success_api_response


def query_filter(fields: List[Tuple[str, Type]],
        ...
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            for field in fields:
                ...
                querys = [(key, query_dict.get(key))
                          ...
                          if key.startswith(field_name + separator)]
                for query in querys:
                    key: str = query[0]
                    # type conversion
                    if type_attr is not str:
                        try:
                            ...
                            return failed_api_response(
                                ErrorCode.INVALID_REQUEST_ARGS,
                                "Sorry, {} should be {}.".format(
                                    field_name, type_attr.__name__))
                    else:
                        value = query[1]
        ...
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            for order_by_value in order_by_values:
                if not order_by_value in allowed_fields:
                    return failed_api_response(
                        ErrorCode.INVALID_REQUEST_ARGS,
                        "Sorry, it is not valid to order by {}.".format(
                            order_by_value))
...
def default_distinct_helper(request: HttpRequest, model: Model, distinct_field,
                            *args, **kwargs):
    """[summary]
    ...
    if model_filter is None:
        ...
        try:
            ...
            return failed_api_response(ErrorCode.INVALID_REQUEST_ARGS,
                                       "Unsupported Filter Method.")
    model_instances = model_instances.order_by(distinct_field).distinct(
        distinct_field)
    distinct_data = list(model_instances.values_list(distinct_field,
                                                     flat=True))
...
def query_distinct(fields: List[str], model: Model = None, func=None):
    ...
    def decorator(function):
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            if not distinct_value in fields:
                return failed_api_response(
                    ErrorCode.INVALID_REQUEST_ARGS,
                    "Sorry, distinct value \"{}\" is not valid.".format(
                        distinct_value))
            if func is None:
                return default_distinct_helper(request, model, distinct_value,
                                               *args, **kwargs)
